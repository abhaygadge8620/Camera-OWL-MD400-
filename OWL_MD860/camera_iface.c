...
{
    return (int)closesocket(OWL_SOCK(fd));
}
...
static PROCESS_INFORMATION g_vlc_pi;
...
static void owl_kill_processes_by_name(const char *exe_name)
{
    ...
    do {
        ...
        proc = OpenProcess(PROCESS_TERMINATE | SYNCHRONIZE, FALSE, pe.th32ProcessID);
        if (proc == NULL) {
            continue;
        }

        (void)TerminateProcess(proc, 0);
        (void)WaitForSingleObject(proc, 3000);
        CloseHandle(proc);
...
static int owl_resolve_vlc_path(const char *configured_path, char *resolved_path, size_t resolved_path_sz)
{
    ...
    if ((configured_path != NULL) && (configured_path[0] != '\0') &&
        (GetFileAttributesA(configured_path) != INVALID_FILE_ATTRIBUTES)) {
        n = (DWORD)snprintf(resolved_path, resolved_path_sz, "%s", configured_path);
        if ((n > 0u) && (n < resolved_path_sz)) {
            return 0;
        }
        return -1;
    }

    n = SearchPathA(NULL, "vlc.exe", NULL, (DWORD)resolved_path_sz, resolved_path, NULL);
    if ((n > 0u) && (n < resolved_path_sz)) {
        return 0;
    }

    if (GetFileAttributesA(VLC_EXE_PATH) != INVALID_FILE_ATTRIBUTES) {
        n = (DWORD)snprintf(resolved_path, resolved_path_sz, "%s", VLC_EXE_PATH);
        if ((n > 0u) && (n < resolved_path_sz)) {
            return 0;
        }
    }
...
//     if ((disable_mask & (OWL_TRK_DISABLE_THERMAL | OWL_TRK_DISABLE_DAY1 | OWL_TRK_DISABLE_DAY2)) == 0u) {
//         return OWL_ERR_ARG;
//     }
...
int owl_cam_day_lowlight_zoom(owl_cam_t *cam, uint8_t zoom_mode, uint8_t direction, uint8_t speed)
{
    ...
    if ((zoom_mode != OWL_ZOOM_MODE_CONTINUOUS) &&
        (zoom_mode != OWL_ZOOM_MODE_MANUAL)) {
        return OWL_ERR_ARG;
    }
    if ((direction != OWL_ZOOM_IN) &&
    ...
    payload[1] = direction;
    payload[2] = speed;
    return owl_cam_write(cam, OWL_IFACE_DAY_LOWLIGHT, OWL_ADDR_ZOOM_CTRL, payload, 3u);
}
...
int owl_cam_day_lowlight_zoom_stop(owl_cam_t *cam)
{
    ...
    return owl_write_u8(cam, OWL_IFACE_DAY_LOWLIGHT, OWL_ADDR_ZOOM_STOP, 0x01u);
}
...
int owl_cam_day_normal_zoom(owl_cam_t *cam, uint8_t zoom_mode, uint8_t direction, uint8_t speed)
{
    ...
    payload[1] = direction;
    payload[2] = speed;
    return owl_cam_write(cam, OWL_IFACE_DAY_NORMAL, OWL_ADDR_ZOOM_CTRL, payload, 3u);
}
...
int owl_cam_day_normal_zoom_stop(owl_cam_t *cam)
{
    ...
    return owl_write_u8(cam, OWL_IFACE_DAY_NORMAL, OWL_ADDR_ZOOM_STOP, 0x01u);
}
...
int owl_cam_thermal_zoom(owl_cam_t *cam, uint8_t zoom_mode, uint8_t direction)
{
    ...
    payload[1] = direction;
    payload[2] = 0x01u;
    return owl_cam_write(cam, OWL_IFACE_THERMAL, OWL_ADDR_ZOOM_CTRL, payload, 3u);
}
...
int owl_cam_thermal_zoom_stop(owl_cam_t *cam)
{
    ...
    return owl_write_u8(cam, OWL_IFACE_THERMAL, OWL_ADDR_ZOOM_STOP, 0x01u);
}
...
void owl_cam_vlc_stop(void)
{
    ...
    if (g_vlc_pi.hProcess != NULL) {
        wait_rc = WaitForSingleObject(g_vlc_pi.hProcess, 0);

        if (wait_rc == WAIT_TIMEOUT) {
            TerminateProcess(g_vlc_pi.hProcess, 0);
            WaitForSingleObject(g_vlc_pi.hProcess, 3000);
        }

        CloseHandle(g_vlc_pi.hProcess);
        g_vlc_pi.hProcess = NULL;
    }

    if (g_vlc_pi.hThread != NULL) {
        CloseHandle(g_vlc_pi.hThread);
        g_vlc_pi.hThread = NULL;
    }

    ZeroMemory(&g_vlc_pi, sizeof(g_vlc_pi));
    g_vlc_started = 0;

    /* Enforce single-instance behavior even for VLC started outside this app. */
    owl_kill_processes_by_name("vlc.exe");
}
int owl_cam_vlc_switch_stream(const char *vlc_path, const char *rtsp_url)
{
    char resolved_vlc_path[MAX_PATH];
    const char *launch_target = "vlc.exe";
    HINSTANCE shell_rc;
    ...
    shell_rc = ShellExecuteA(NULL, "open", launch_target, rtsp_url, NULL, SW_SHOWNORMAL);
    if ((INT_PTR)shell_rc <= 32) {
        return OWL_ERR_IO;
    }
