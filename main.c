...
static owl_rtsp_urls_t g_rtsp_urls;
static int g_rtsp_urls_valid = 0;
...
static int cache_rtsp_urls(owl_cam_t *cam, unsigned long loop_count, const char *phase_tag)
{
    char rtsp_blob[256];
    int rc;
    ...
    memset(&g_rtsp_urls, 0, sizeof(g_rtsp_urls));
    g_rtsp_urls_valid = 0;

    rc = owl_cam_get_rtsp_url(cam, rtsp_blob, sizeof(rtsp_blob));
    if (rc != OWL_OK) {
        MAIN_LOG("loop=%lu %s read failed rc=%d", loop_count, phase_tag, rc);
        return rc;
    }

    rc = owl_cam_parse_rtsp_urls(rtsp_blob, &g_rtsp_urls);
    if (rc != OWL_OK) {
        MAIN_LOG("loop=%lu %s parse failed rc=%d blob='%s'", loop_count, phase_tag, rc, rtsp_blob);
        return rc;
    }

    g_rtsp_urls_valid = 1;
    MAIN_LOG("loop=%lu %s cached thermal='%s' day='%s' low_light='%s'",
             loop_count,
             phase_tag,
             g_rtsp_urls.thermal,
             g_rtsp_urls.day,
             g_rtsp_urls.low_light);
    return OWL_OK;
}
...
static BOOL WINAPI on_console_ctrl(DWORD ctrl_type)
{
    ...
    case CTRL_SHUTDOWN_EVENT:
        MAIN_LOG("console control event received type=%lu, stopping loop", (unsigned long)ctrl_type);
        (void)InterlockedExchange(&g_running, 0);
        return TRUE;
    default:
        MAIN_LOG("console control event ignored type=%lu", (unsigned long)ctrl_type);
        return FALSE;
    }
}
...
static int parse_bool_string(const char *s, bool *out)
{
    ...
    if ((strcmp(s, "1") == 0) || (_stricmp(s, "true") == 0) || (_stricmp(s, "yes") == 0) || (_stricmp(s, "on") == 0)) {
        *out = true;
        MAIN_LOG("parse_bool_string value='%s' parsed=true", s);
        return 0;
    }
    if ((strcmp(s, "0") == 0) || (_stricmp(s, "false") == 0) || (_stricmp(s, "no") == 0) || (_stricmp(s, "off") == 0)) {
        *out = false;
...
static int load_camera_settings(const char *path, camera_params_t *params, uint16_t *tcp_port_out, bool *telemetry_enable_out)
{
    ...
    *telemetry_enable_out = false;
    ...
    if (ini_get_value(path, "udp_multicast", "enabled", value, sizeof(value)) == 0) {
        (void)parse_bool_string(value, &params->udp_mcast_enabled);
        MAIN_LOG("udp_multicast.enabled=%d", params->udp_mcast_enabled ? 1 : 0);
    }
    if (ini_get_value(path, "udp_multicast", "group_ip", value, sizeof(value)) == 0) {
        ...
        MAIN_LOG("udp_multicast.group_ip='%s'", params->udp_mcast_group_ip);
    }
    if (ini_get_value(path, "udp_multicast", "port", value, sizeof(value)) == 0) {
        (void)parse_u16_string(value, &params->udp_mcast_port);
        MAIN_LOG("udp_multicast.port=%u", (unsigned)params->udp_mcast_port);
    }
    if (ini_get_value(path, "udp_multicast", "iface_ip", value, sizeof(value)) == 0) {
        ...
        MAIN_LOG("udp_multicast.iface_ip='%s'", params->udp_mcast_iface_ip);
    }
    if (ini_get_value(path, "udp_multicast", "ttl", value, sizeof(value)) == 0) {
        uint16_t parsed = 0u;
        if (parse_u16_string(value, &parsed) == 0) {
            params->udp_mcast_ttl = (uint8_t)parsed;
        }
        MAIN_LOG("udp_multicast.ttl=%u", (unsigned)params->udp_mcast_ttl);
    }
    if (ini_get_value(path, "udp_multicast", "loopback", value, sizeof(value)) == 0) {
        bool loopback = false;
        if (parse_bool_string(value, &loopback) == 0) {
            params->udp_mcast_loopback = loopback ? 1u : 0u;
        }
        MAIN_LOG("udp_multicast.loopback=%u", (unsigned)params->udp_mcast_loopback);
    }
    if (ini_get_value(path, "udp_multicast", "camera_id", value, sizeof(value)) == 0) {
        uint16_t parsed = 0u;
        if (parse_u16_string(value, &parsed) == 0) {
            params->udp_camera_id = (uint8_t)parsed;
        }
        MAIN_LOG("udp_multicast.camera_id=%u", (unsigned)params->udp_camera_id);
    }
    if (ini_get_value(path, "camera", "telemetry_enabled", value, sizeof(value)) == 0) {
        (void)parse_bool_string(value, telemetry_enable_out);
    } else if (ini_get_value(path, "udp_multicast", "enabled", value, sizeof(value)) == 0) {
        (void)parse_bool_string(value, telemetry_enable_out);
    }
                                     ...
                                     const char *reason)
{
    ...
    (void)cache_rtsp_urls(cam, loop_count, "camera recover rtsp");
                                             ...
                                             unsigned long loop_count)
{
    ...
    if ((g_rtsp_urls_valid == 0) || (selected_rtsp == NULL) || (selected_rtsp[0] == '\0')) {
        MAIN_LOG("loop=%lu rtsp switch cannot run, urls_valid=%d selected_button=%s",
                 loop_count, g_rtsp_urls_valid, selected_button ? selected_button : "NULL");
        return -3;
    }
    ...
    rc = owl_cam_vlc_switch_stream(VLC_EXE_PATH, selected_rtsp);
    if (rc != OWL_OK) {
        MAIN_LOG("loop=%lu rtsp vlc switch failed id=%u value=%u button=%s rc=%d",
                 loop_count, (unsigned)id, (unsigned)value, selected_button, rc);
        return -6;
    }

    g_selected_view_cam = cam_id;

    MAIN_LOG("loop=%lu rtsp vlc switch ok id=%u value=%u button=%s url=%s selected_cam=%u tracker_mode_on=%d",
             loop_count,
             (unsigned)id,
             (unsigned)value,
             selected_button,
...
int main(void)
{
    ...
    DWORD lrf_reset_clear_deadline = 0u;
    DWORD optics_reset_clear_deadline = 0u;
    DWORD last_telem_tick = 0u;
    ...
    if (cam_cfg.udp_mcast_enabled) {
        ...
        memset(&api2_cfg, 0, sizeof(api2_cfg));
        (void)strncpy(api2_cfg.group_ip, cam_cfg.udp_mcast_group_ip, sizeof(api2_cfg.group_ip) - 1u);
        api2_cfg.group_ip[sizeof(api2_cfg.group_ip) - 1u] = '\0';
        (void)strncpy(api2_cfg.iface_ip, cam_cfg.udp_mcast_iface_ip, sizeof(api2_cfg.iface_ip) - 1u);
        api2_cfg.iface_ip[sizeof(api2_cfg.iface_ip) - 1u] = '\0';
        api2_cfg.port = cam_cfg.udp_mcast_port;
        api2_cfg.ttl = cam_cfg.udp_mcast_ttl;
        api2_cfg.loopback = cam_cfg.udp_mcast_loopback;

        rc = api2_init(&api2_cfg);
        if (rc == 0) {
            api2_open = 1;
            camera_iface_set_udp_mcast(true, cam_cfg.udp_camera_id, &udp_mcast_seq);
        } else {
    ...
    /*
     * New behavior:
     * Do NOT auto-start tracker here.
     * Instead read RTSP URLs once and keep tracker OFF.
     */
    ...
    if (telemetry_enable) {
        rc = camera_telem_open(&telem_ctx, "0.0.0.0", 0u);
        if (rc == 0) {
            telem_open = 1;
            last_telem_tick = GetTickCount();
        } else {
    ...
    while (InterlockedCompareExchange(&g_running, 1, 1) != 0) {
        ...
        if (telem_open != 0) {
            const int trc = camera_telem_recv(&telem_ctx, &telem, 0);
            if (trc > 0) {
                last_telem_tick = GetTickCount();
                ...
                if ((LONG)(now - last_telem_tick) >= (LONG)TELEMETRY_LOSS_LED_OFF_MS) {
        ...
        if (lrf_reset_clear_deadline != 0u) {
            const DWORD now = GetTickCount();
            if ((LONG)(now - lrf_reset_clear_deadline) >= 0) {
        ...
        if (optics_reset_clear_deadline != 0u) {
            const DWORD now = GetTickCount();
            if ((LONG)(now - optics_reset_clear_deadline) >= 0) {
    ...
    /* Stop VLC opened by this app */
    owl_cam_vlc_stop();

    uart_close(&uart);

    if ((g_tracker_mode_on != 0) && (g_last_tracker_valid != 0)) {
        const int trk_rc = main_tracker_set_coord(&cam,
                                                  OWL_IFACE_TRACKER,
...
// #define MAIN_LOG(fmt, ...) do { printf("[MAIN] " fmt "\n", __VA_ARGS__); } while (0)
// #define MAIN_LOG0(msg) do { printf("[MAIN] %s\n", msg); } while (0)
...
//         (void)parse_bool_string(value, telemetry_enable_out);
//     } else if (ini_get_value(path, "udp_multicast", "enabled", value, sizeof(value)) == 0) {
//         /* Reuse existing config key as fallback when telemetry_enabled is not present. */
//         (void)parse_bool_string(value, telemetry_enable_out);
//     }
...
//     return 0;
// }
// //tracker mode continuous on with retry
// static int enable_tracker_mode_on_with_retry(owl_cam_t *cam, unsigned long loop_count, const char *phase_tag)
// {
//     int trk_rc = OWL_ERR_IO;

//     if ((cam == NULL) || (phase_tag == NULL)) {
//         if (cam == NULL) {
//             return OWL_ERR_ARG;
//         }
//         phase_tag = "tracker";
...
//                                         OWL_IFACE_TRACKER,
//                                         OWL_TRACKER_DEFAULT_CAM,
//                                         OWL_TRACKER_DEFAULT_X,
//                                         OWL_TRACKER_DEFAULT_Y,
//                                         OWL_TRACKER_START);
//         log_camera_response(phase_tag, trk_rc);
//         if (trk_rc == OWL_OK) {
//             g_last_tracker_state.coord_cam_id = OWL_TRACKER_DEFAULT_CAM;
//             g_last_tracker_state.coord_x = OWL_TRACKER_DEFAULT_X;
//             g_last_tracker_state.coord_y = OWL_TRACKER_DEFAULT_Y;
//             g_last_tracker_state.coord_mode = OWL_TRACKER_START;
//             g_last_tracker_valid = 1;

//             MAIN_LOG("loop=%lu %s mode on cam=%u x=%u y=%u attempt=%d",
//                      loop_count,
//                      phase_tag,
//                      (unsigned)OWL_TRACKER_DEFAULT_CAM,
//                      (unsigned)OWL_TRACKER_DEFAULT_X,
//                      (unsigned)OWL_TRACKER_DEFAULT_Y,
//                      attempt);
//             return OWL_OK;
//         }
...
//         MAIN_LOG("loop=%lu camera recover telemetry set failed rc=%d", loop_count, rc);
//     }
//     //tracker functions are the most likely to hit OWL_ERR_IO, so we will attempt to re-enable tracker mode on recovery before returning failure, but we won't fail the recovery if that doesn't work since the camera connection itself is healthy at this point.
//     (void)enable_tracker_mode_on_with_retry(cam, loop_count, "camera recover tracker");

//     MAIN_LOG("loop=%lu camera recover done", loop_count);
//     return OWL_OK;
// }
// //tracker function for reconnection automatic
// static int call_cam_u8_with_recover(int (*fn)(owl_cam_t *, uint8_t),
//                                     owl_cam_t *cam,
//                                     uint8_t arg,
//                                     const camera_params_t *cam_cfg,
...
//     MAIN_LOG("loop=%lu %s hit OWL_ERR_IO, attempting reconnect", loop_count, op_name);
//     if (recover_camera_connection(cam, cam_cfg, cam_tcp_port, telemetry_enable, loop_count, op_name) != OWL_OK) {
//         return rc;
//     }
//     return fn(cam, arg);
// }
...
//                                   "owl_cam_tracker_switch_camera");
//     if (rc != OWL_OK) {
//         MAIN_LOG("loop=%lu view switch failed id=%u cam_id=%u rc=%d",
//                  loop_count, (unsigned)id, (unsigned)cam_id, rc);
//         return -2;
//     }

//     MAIN_LOG("loop=%lu view switch ACK ok id=%u value=%u cam_id=%u",
//              loop_count, (unsigned)id, (unsigned)value, (unsigned)cam_id);

//     led_rc = led_status_router_set_led(led_router, "DAY", (strcmp(selected_button, "DAY") == 0) ? 1u : 0u);
//     if (led_rc < 0) {
//         MAIN_LOG("loop=%lu led update failed for DAY rc=%d", loop_count, led_rc);
//     }
//     led_rc = led_status_router_set_led(led_router, "LOW_LIGHT", (strcmp(selected_button, "LOW_LIGHT") == 0) ? 1u : 0u);
//     if (led_rc < 0) {
//         MAIN_LOG("loop=%lu led update failed for LOW_LIGHT rc=%d", loop_count, led_rc);
//     }
//     led_rc = led_status_router_set_led(led_router, "THERMAL", (strcmp(selected_button, "THERMAL") == 0) ? 1u : 0u);
//     if (led_rc < 0) {
//         MAIN_LOG("loop=%lu led update failed for THERMAL rc=%d", loop_count, led_rc);
//     }
...
//     switch (value) {
//     case 1u:
//         freq_mode = OWL_LRF_FREQ_1HZ;
//         break;
//     case 4u:
//         freq_mode = OWL_LRF_FREQ_4HZ;
//         break;
//     case 10u:
//         freq_mode = OWL_LRF_FREQ_10HZ;
//         break;
//     case 20u:
//         freq_mode = OWL_LRF_FREQ_20HZ;
//         break;
//     case 100u:
//         freq_mode = OWL_LRF_FREQ_100HZ;
//         break;
//     case 200u:
...
//     if ((cam == NULL) || (uart == NULL) || (cam_cfg == NULL)) {
//         return -1;
//     }
...
//         return 1;
//     }
//     MAIN_LOG("UART open ok device=%s baud=%lu", uart_cfg.uart_device, (unsigned long)uart_cfg.uart_baud);

//     MAIN_LOG("opening camera connection ip=%s port=%u...", cam_cfg.camera_ip, (unsigned)cam_tcp_port);
//     rc = owl_cam_open(&cam, cam_cfg.camera_ip, cam_tcp_port);
//     if (rc != 0) {
//         MAIN_LOG("error: owl_cam_open ip=%s port=%u rc=%d", cam_cfg.camera_ip, (unsigned)cam_tcp_port, rc);
//         uart_close(&uart);
//         return 1;
//     }
...
//     (void)enable_tracker_mode_on_with_retry(&cam, g_loop_count, "startup tracker");

//     if (run_camera_diag_on_start != 0) {
//         MAIN_LOG0("camera diagnostics start");
//         MAIN_LOG0("diag step 1/4: read power controls");
//         rc = camera_diag_read_power_controls(&cam);
//         MAIN_LOG("diag step 1/4 rc=%d", rc);

//         MAIN_LOG0("diag step 2/4: reboot DAY");
//         rc = camera_diag_reboot_day(&cam);
//         MAIN_LOG("diag step 2/4 rc=%d", rc);

//         MAIN_LOG0("diag step 3/4: reboot THERMAL");
//         rc = camera_diag_reboot_thermal(&cam);
//         MAIN_LOG("diag step 3/4 rc=%d", rc);

//         MAIN_LOG0("diag step 4/4: reboot LRF");
//         rc = camera_diag_reboot_lrf(&cam);
//         MAIN_LOG("diag step 4/4 rc=%d", rc);
//         MAIN_LOG0("camera diagnostics done");
//     }
...
//             MAIN_LOG("loop=%lu frame decoded id=%u value=%u",
//                      g_loop_count, (unsigned)rx_id, (unsigned)rx_value);
//             lrf_single_measure_rc = handle_lrf_single_measure_mode_command(&cam, &cam_cfg, cam_tcp_port, telemetry_enable, rx_id, rx_value, g_loop_count);
//             if (lrf_single_measure_rc < 0) {
//                 MAIN_LOG("loop=%lu lrf single measure handler error id=%u value=%u rc=%d",
//                          g_loop_count, (unsigned)rx_id, (unsigned)rx_value, lrf_single_measure_rc);
//             } else if (lrf_single_measure_rc == 1) {
...
//             lrf_freq_rc = handle_lrf_frequency_command(&cam, &cam_cfg, cam_tcp_port, telemetry_enable, rx_id, rx_value, g_loop_count);
//             if (lrf_freq_rc < 0) {
//                 MAIN_LOG("loop=%lu lrf frequency handler error id=%u value=%u rc=%d",
//                          g_loop_count, (unsigned)rx_id, (unsigned)rx_value, lrf_freq_rc);
//             } else if (lrf_freq_rc == 1) {
...
//             lrf_align_rc = handle_lrf_align_pointer_command(&cam, &uart, &cam_cfg, cam_tcp_port, telemetry_enable, rx_id, rx_value, g_loop_count);
//             if (lrf_align_rc < 0) {
//                 MAIN_LOG("loop=%lu lrf align pointer handler error id=%u value=%u rc=%d",
//                          g_loop_count, (unsigned)rx_id, (unsigned)rx_value, lrf_align_rc);
//             } else if (lrf_align_rc == 1) {
...
//                          g_loop_count, (unsigned)rx_id, (unsigned)rx_value, switch_rc);
//             } else if (switch_rc == 1) {
//                 MAIN_LOG("loop=%lu camera view switch handler completed id=%u value=%u",
//                          g_loop_count, (unsigned)rx_id, (unsigned)rx_value);
//             } else {
//                 MAIN_LOG("loop=%lu no direct camera view mapping for id=%u value=%u",
//                          g_loop_count, (unsigned)rx_id, (unsigned)rx_value);
...
//     {
//         tracker_state_t trk_state;
//         int got_state = 0;

//         for (int attempt = 1; attempt <= 3; ++attempt) {
//             rc = camera_iface_read_tracker(&trk_state);
//             if (rc == OWL_OK) {
//                 got_state = 1;
//                 break;
//             }
...
//         if ((got_state == 0) && (g_last_tracker_valid != 0)) {
//             trk_state = g_last_tracker_state;
//             got_state = 1;
//         }
...
//             const int trk_rc = main_tracker_set_coord(&cam,
//                                                       OWL_IFACE_TRACKER,
//                                                       trk_state.coord_cam_id,
//                                                       trk_state.coord_x,
//                                                       trk_state.coord_y,
//                                                       OWL_TRACKER_STOP);
//             log_camera_response("shutdown tracker", trk_rc);
//             MAIN_LOG("shutdown tracker mode off cam=%u x=%u y=%u",
//                      (unsigned)trk_state.coord_cam_id,
//                      (unsigned)trk_state.coord_x,
//                      (unsigned)trk_state.coord_y);
//         }
//     }
